...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("jump") and current_phase == Phase.WHITE and !(is_on_floor() or !$coyote_timer.is_stopped()):
	...
	if current_phase == Phase.PHOENIX:
		if $phoenix_timer.is_stopped() and phoenix_check == 1:
			current_phase = Phase.BLACK
			$AnimatedSprite2D.play("black")
			phoenix_check = 0
		if $phoenix_timer.is_stopped():
			$phoenix_timer.start()
			phoenix_check = phoenix_check +1
			print(phoenix_check)
...
func _on_area_2d_body_entered(body: Node2D) -> void:
	current_phase = Phase.PHOENIX
	$AnimatedSprite2D.play("phoenix")
